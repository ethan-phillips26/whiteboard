// api.js for the browser build: the same surface as the server's, answered in the
// page. Vite swaps this in for ../api.js when building with `--mode browser`, so
// every screen imports "api.js" and gets whichever this build is. What app.py
// does per route happens here per function, against the same shapes.

import { save } from "../lib/download.js";
import { ask, present } from "./bridge.js";
import * as E from "./edits.js";
import * as G from "./grades.js";
import * as ics from "./ics.js";
import * as N from "./notices.js";
import * as store from "./store.js";
import * as sync from "./sync.js";

/** No Google connection — its OAuth flow needs a server to hold the secret — and
 * sign-in through the extension rather than a desktop window. */
export const features = { google: false, login: "extension" };

/** An error shaped like the server's, so callers can read `.status`. */
...
const unavailable = async () => {
  throw failure("Google Docs needs the desktop app.", 501);
};
...
export const api = {
  ...
  desktopLogin: async () => {
    throw failure("The window sign-in needs the desktop app.", 409);
  },
  ...
  googleStatus: async () => ({ configured: false, authorised: false }),
  googleSaveCredentials: unavailable,
  googleForget: unavailable,
  googleAuthorise: unavailable,
  googleDisconnect: unavailable,
  openInDocs: unavailable,
  ...
  // Local corrections. These never reach Blackboard.
  edits: () => E.load(),
