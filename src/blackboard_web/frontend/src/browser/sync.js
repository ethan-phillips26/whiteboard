// sync.py for the browser build: fetch through the extension, remember everything
// in IndexedDB, and only ask Blackboard again when an entry goes stale. The shapes
// written and returned are the server's, field for field, so every screen reads
// them without knowing which build it is in.
...
/** list_due_dates, over bundles already fetched rather than a second round of
 * requests: what is outstanding, due from three days ago to sixty days out. */
...
/** The student's percentages as fractions, for categories this course has.
 * See sync.category_weights. */
...
// The shape of a cached assignment detail; bump with the shape. See app.py.
...
// type from an object URL it would execute in this page's origin. See AS_TEXT in
// app.py.
